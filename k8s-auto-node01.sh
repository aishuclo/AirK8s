...
NODE01_NAME=node01
NODE01_IP=192.168.228.152
NODE_IP_MASK=24

# 1. 设置主机名
hostnamectl set-hostname $NODE01_NAME 
sed -i '/^127.0.1.1/d' /etc/hosts 
echo "127.0.1.1 $NODE01_NAME" >> /etc/hosts


# 2. 设置静态网络（重启不会改变IP）
nmcli connection modify ens33 ipv4.method manual ipv4.address $NODE01_IP/$NODE_IP_MASK ipv4.gateway $GATE_WAY ipv4.dns $DNS 
nmcli connection up ens33

# 3. 增加air-k8s-server的映射
sed -i '/air-k8s-server/d' /etc/hosts
echo "$AIR_K8S_SERVER air-k8s-server" >> /etc/hosts
...
[DVD]
name=DVD
baseurl=http://air-k8s-server/dvd/
gpgcheck=1
enabled=1
gpgkey=http://air-k8s-server/dvd/RPM-GPG-KEY-openEuler
EOF
...
[MySoft]
name=MySoft
baseurl=http://${AIR_K8S_SERVER}/mysoft/
gpgcheck=0
enabled=1
EOF
...
# 7. 安装nerdctl
mkdir -p ~/nerdctl
wget http://${AIR_K8S_SERVER}/k8s/nerdctl-linux-amd64.tar.gz -O ~/nerdctl/nerdctl-linux-amd64.tar.gz
tar -xzf ~/nerdctl/nerdctl-linux-amd64.tar.gz -C ~/nerdctl/ && chmod +x ~/nerdctl/nerdctl
